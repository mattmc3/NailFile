...
#         reader = data_reader.FixedWidthReader(fn_generator, (5, 30, 1, 8, 20),
#                                               ('person_num', 'name', 'gender', 'dob', 'relationship'))
#         loader = text_query.DataLoader()
...
#         reader = data_reader.CollectionReader(fn, headers=False)
#         loader = text_query.DataLoader()
#         ds = loader.load(reader)
...
#         csv_reader = data_reader.CsvReader('test_data.csv')
#         loader = text_query.DataLoader()
#         ds = loader.load(csv_reader)
#         return ds
#
...
#         reader = data_reader.CollectionReader(fn_get_list_data)
#         loader = text_query.DataLoader()
...
#
#     def test_csv_loads_seven_huxtables(self):
#         ds = self.load_csv_data()
#         count = ds.scalar('select count(*) from tbl')
#         self.assertEqual(count, 7)
#
#     def test_csv_loads_correct_data(self):
#         ds = self.load_csv_data()
#         guys = ds.fetchall('select * from tbl where gender = ? order by name desc', params=('M',))
#         self.assertEqual(len(guys), 2)
#         self.assertTrue(guys[0].name.startswith('Theo'))
#         self.assertTrue(guys[1].name.startswith('Cliff'))
